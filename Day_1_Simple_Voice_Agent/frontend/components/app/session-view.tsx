...
function PlayerBadge({ participant }: { participant?: LocalParticipant }) {
    // === ADDED: Load name from backend JSON ===
  const backendName = useBackendParticipantName();
  ...
  useEffect(() => {
    ...
    const updateName = () => {
      ...
      // === MODIFIED: If backend contains user's name, override with backend name ===
      if (backendName) {
...
// === ADDED: Fetch saved participant records from backend JSON ===
function useBackendParticipantName() {
  const [backendName, setBackendName] = useState<string | null>(null);

  useEffect(() => {
    async function loadName() {
      try {
        const res = await fetch("http://localhost:3000/get_participants"); 
        ...
        // JSON structure: { participants: [ { name: "Sujal", sessionId: "xyz" }, ... ] }
        if (data?.participants?.length > 0) {
          // Get the most recent participant
          const last = data.participants[data.participants.length - 1];
          if (last?.name) setBackendName(last.name);
        }
      } catch (err) {
        console.error("Could not load participant JSON:", err);
      }
    }
