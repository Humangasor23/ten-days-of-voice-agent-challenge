...
# Loads catalogue from catalogue.json.
...
CATALOG = load_catalogue().get("products", [])
...
async def show_catalog(
    ...
    # Summarize top 8
    lines = [f"Here are the top {min(8, len(prods))} items I found at Sujal's Shop:"]
...
async def show_catalog(
    ...
    if not prods:
        return "Sorry — I couldn't find any items that match. Would you like to try another search?"
    # Summarize top 4
    lines = [f"Here are the top {min(4, len(prods))} items I found at Sujal's Shop:"]
...
class GameMasterAgent(Agent):
    def __init__(self):
        ...
        instructions = """
        You are 'Ramu Kaka', the friendly shopkeeper and voice assistant for Sujal's Shop.
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("🚀 STARTING VOICE E-COMMERCE AGENT (Sujal's Shop) — Ramu Kaka")
